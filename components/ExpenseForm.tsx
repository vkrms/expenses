    ...
    <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 max-w-lg mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
          <button
            onClick={() => setMode('ai')}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${mode === 'ai' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            <span className="flex items-center gap-1"><Sparkles className="w-3 h-3" /> Smart</span>
          </button>
          <button
            onClick={() => setMode('manual')}
            className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${mode === 'manual' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
          >
            Manual
          </button>
        </div>
      ...
      {mode === 'manual' ? (
        <form onSubmit={handleManualSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {Object.values(ExpenseCategory).map(cat => (
                <button
                  ...
                  className={`text-xs py-2 px-1 rounded-lg border transition-all ${category === cat ? 'bg-emerald-50 border-emerald-500 text-emerald-700 font-semibold' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}
                >
                  {cat}
                </button>
              ))}
          ...
          <div className="pt-4 flex gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 rounded-xl bg-emerald-500 text-white font-medium hover:bg-emerald-600 transition-colors disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-emerald-200"
            >
        ...
        <form onSubmit={handleAiSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Tell me what you bought</label>
            <textarea
              required
              rows={4}
              value={aiInput}
            ...
            />
            <p className="mt-2 text-xs text-gray-400">Powered by Gemini. Can detect multiple items.</p>
          </div>
          <div className="pt-4 flex gap-3">
            <button
              type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-medium hover:from-emerald-600 hover:to-teal-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-emerald-200"
            >
